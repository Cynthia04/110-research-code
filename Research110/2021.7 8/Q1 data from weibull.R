...
ll1 <- function(theta,data){
  ll(theta[1]*10^(-2),theta[2]*10^(-4),theta[3],data)
}
set.seed(109225017);r1 <- runif(50,0,10);r2 <- runif(50,0,10);r3 <- runif(50,0,100)
...
ll1.log <- function(theta,data){
  ll.log(theta[1]*10^(-2),theta[2]*10^(-4),theta[3],data)
}
...
system.time({
  for(k in 1:8){
    optim.result=matrix(0,nrow = 1,ncol = 4)
    for (i in 1:50){
      fit=try(optim(c(r1[i],r2[i],r3[i]),ll1.log,hessian=T,data=Ti[,k]))
      if(class(fit)=="try-error"){
        next
      }
      fit1=try(optim(fit$par,ll1.log,hessian=T,data=Ti[,k]))
...
ll1.wei <- function(theta,data){
  ll.wei(theta[1]*10^(-2),theta[2]*10^(-4),theta[3],data)
}
set.seed(10901);r1 <- runif(50,0,10);r2 <- runif(50,0,10);r3 <- runif(50,0,100)
table2 <- matrix(0,nrow = 8,ncol =4)
system.time({
  for(k in 1:8){
    optim.result=matrix(0,nrow = 1,ncol = 4)
