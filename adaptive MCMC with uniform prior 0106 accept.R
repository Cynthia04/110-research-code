#================================================
# adaptive MCMC with uniform prior 0106 accept.R
#================================================

Ti <- read.csv("data.csv")
Ti <- rbind(rep(0,9),Ti)
sk <- c(0.111,0.556,1)          # standardize stress  !!!!!
n1 <- 3 ;n2 <- 3 ;n3 <- 2
nk <- n1+n2+n3
Ti <- Ti[,-9]
...
tll1.wei <- function(theta,data){
  -tll.wei(theta[1],theta[2],theta[3],theta[4],theta[5],theta[6],data) ## logL here !!!
}
# mle from weibull result
t3.wei <- as.matrix(read.csv("mle.csv")[,-1],ncol=1)

#--------------------------- mcmc alpha function for Weibull model --------------------------
...
alphalikelihood1.1<- function(theta,data){ 
  ...
  bS <- apply(matrix(sk,ncol = 1),1,function(x) b0+b1*x)
  sS <- apply(matrix(sk,ncol = 1),1,function(x) s0+s1*x)
  t1 <- sum(apply(matrix(1:n1,ncol = 1),1,function(x) alphalikelihood1(aS[1],bS[1],sS[1],data[,x])))
  t2 <- sum(apply(matrix((n1+1):(n1+n2),ncol = 1),1,function(x) alphalikelihood1(aS[2],bS[2],sS[2],data[,x])))
  t3 <- sum(apply(matrix((n1+n2+1):nk,ncol = 1),1,function(x) alphalikelihood1(aS[3],bS[3],sS[3],data[,x])))
  ans <- t1+t2+t3
  ans
}
...
alphalikelihood2.1<- function(theta,data){ 
  ...
  bS <- apply(matrix(sk,ncol = 1),1,function(x) b0+b1*x)
  sS <- apply(matrix(sk,ncol = 1),1,function(x) s0+s1*x)
  t1 <- sum(apply(matrix(1:n1,ncol = 1),1,function(x) alphalikelihood2(aS[1],bS[1],sS[1],data[,x])))
  t2 <- sum(apply(matrix((n1+1):(n1+n2),ncol = 1),1,function(x) alphalikelihood2(aS[2],bS[2],sS[2],data[,x])))
  t3 <- sum(apply(matrix((n1+n2+1):nk,ncol = 1),1,function(x) alphalikelihood2(aS[3],bS[3],sS[3],data[,x])))
  ans <- t1+t2+t3
  ans
}
alphalikelihood3 = function(a,b,c,data){  
  ...
  ans= 45*lgamma(1/c)+sum(k)-sum(k1)
  ans
}
alphalikelihood3.1<- function(theta,data){ 
  ...
  bS <- apply(matrix(sk,ncol = 1),1,function(x) b0+b1*x)
  sS <- apply(matrix(sk,ncol = 1),1,function(x) s0+s1*x)
  t1 <- sum(apply(matrix(1:n1,ncol = 1),1,function(x) alphalikelihood3(aS[1],bS[1],sS[1],data[,x])))
  t2 <- sum(apply(matrix((n1+1):(n1+n2),ncol = 1),1,function(x) alphalikelihood3(aS[2],bS[2],sS[2],data[,x])))
  t3 <- sum(apply(matrix((n1+n2+1):nk,ncol = 1),1,function(x) alphalikelihood3(aS[3],bS[3],sS[3],data[,x])))
  ans <- t1+t2+t3
  ans
}
#---------------------------------------
# adaptive mcmc with uniform prior
#---------------------------------------
...
#----------------------------------- mcmc table ---------------------------------
#--------------------------------- uniform prior -------------------------------
# seq(m+k,m+N*k,k): use posterior sample
...
mcmc_table
# (mean-mle)^2
(mcmc_table[,4]-mcmc_table[,5])^2
write.csv(mcmc_table,"0106mcmc3_accept.csv")
...
par(mfrow=c(1,1))
# ---------------------------- collect different chain --------------------------------------
chain1 <- data.frame(a0=a0,a1=a1,b0=b0,b1=b1,s0=s0,s1=s1)
chain1 <- rbind(c(initial[1]*10^(-2),initial[2]*10^(-3),initial[3]*10^(-4),initial[4]*10^(-5),
                  initial[5:6]),chain1)
write.csv(chain1,"0106chain1_accept.csv")
chain2 <- data.frame(a0=a0,a1=a1,b0=b0,b1=b1,s0=s0,s1=s1)
chain2 <- rbind(c(initial[1]*10^(-2),initial[2]*10^(-3),initial[3]*10^(-4),initial[4]*10^(-5),
                  initial[5:6]),chain2)
write.csv(chain2,"0106chain2_accept.csv")
chain3 <- data.frame(a0=a0,a1=a1,b0=b0,b1=b1,s0=s0,s1=s1)
chain3 <- rbind(c(initial[1]*10^(-2),initial[2]*10^(-3),initial[3]*10^(-4),initial[4]*10^(-5),
                  initial[5:6]),chain3)
write.csv(chain3,"0106chain3_accept.csv")

library(arm)
r1 <- c(3,4,0,0,5,1,1,1,1,0)
discrete.histogram (r1)
lines(density(r1),
      lwd = 2, # thickness of line
      col = "red")
